...
select distinct[State] from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_LOCATION as b
on a.IDLocation=b.IDLocation
where Year(date) in (select year(date) as [Year] from dbo.FACT_TRANSACTIONS
...
select top 1 State , count(IDCustomer) as Total from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_MODEL as b
on a.IDModel=b.IDModel
inner join dbo.DIM_MANUFACTURER as c
on b.IDManufacturer=c.IDManufacturer
inner join dbo.DIM_LOCATION as d
on a.IDLocation=d.IDLocation
where state in (select distinct [State] from dbo.DIM_LOCATION
...
select a.IDModel , count(IDCustomer) as Count_ , State , ZipCode from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_MODEL as b
on a.IDModel=b.IDModel
inner join dbo.DIM_LOCATION as c
on a.IDLocation=c.IDLocation
group by a.IDModel , State , ZipCode
...
select top 1 * from dbo.DIM_MODEL as a
inner join dbo.DIM_MANUFACTURER as b
...
select Model_Name , b.IDModel , avg(TotalPrice) as Avg_ ,Manufacturer_Name from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_MODEL as b
on a.IDModel=b.IDModel
inner join dbo.DIM_MANUFACTURER as c
on b.IDManufacturer=c.IDManufacturer
where Manufacturer_Name in (select top 5  Manufacturer_Name from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_MODEL as b
on a.IDModel=b.IDModel
inner join dbo.DIM_MANUFACTURER as c
on b.IDManufacturer=c.IDManufacturer
...
group by Model_Name , b.IDModel , Manufacturer_Name
order by Avg_ desc
...
select a.IDCustomer , Customer_Name , avg(TotalPrice) as Avg_ from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_CUSTOMER as b
on a.IDCustomer=b.IDCustomer
where year(date)=2009 
group by a.IDCustomer , Customer_Name
...
select * from
(select top 5 a.IDModel , Model_Name from dbo.FACT_TRANSACTIONS as a
inner join dbo.DIM_MODEL as b
on a.IDModel=b.IDModel
where year(date)=2008
group by a.IDModel , Model_Name , Year(Date)
...
select * from (
select top 1 * from (
select top 2  b.IDManufacturer , Manufacturer_Name , sum(TotalPrice) as Total_Sales , year(date) as [Year] from dbo.FACT_TRANSACTIONS as a
inner join DIM_MODEL as b
on a.IDModel=b.IDModel
inner join DIM_MANUFACTURER as c
on b.IDManufacturer=c.IDManufacturer
where Year(date)=2009
group by b.IDManufacturer , Manufacturer_Name , year(date)
order by Total_Sales desc) as t1
order by Total_Sales) as a1
union all
select * from (
select top 1 * from (
select top 2  b.IDManufacturer , Manufacturer_Name , sum(TotalPrice) as Total_Sales , year(date) as [Year] from dbo.FACT_TRANSACTIONS as a
...
select Manufacturer_Name from FACT_TRANSACTIONS as a
inner join DIM_MODEL as b
on a.IDModel=b.IDModel
inner join DIM_MANUFACTURER as c
on b.IDManufacturer=c.IDManufacturer
...
select *, ((avg_ - lag_)/lag_)*100 as [Percent Change] from (
select *, lag(Avg_,1) over (partition by idcustomer order by year) as lag_ from (
select idcustomer,year(date) as [Year],avg(TotalPrice) as Avg_,sum(Quantity) as Total from FACT_TRANSACTIONS
